...
//   const depth = useSubscribeDepth(productId);
...
const Orderbook = ({ productId }) => {
  ...
  // ✅ Lấy depth từ React Query cache
  const depth = useSubscribeDepth(productId);
  const sellOrders = depth.asks
  ...
  useSubscribeMarketTrade(productId, (tradeData) => {
    setRecentTrade({
      price: tradeData.price,
      amount: tradeData.quantity,
      time: new Date(tradeData.tradeTime).toLocaleTimeString('en-GB'),
      type: tradeData.isMaker ? 'sell' : 'buy',
    });
  });
  ...
  const renderRow = (order, isBuy) => {
    ...
    return (
      ...
      >
        ...
        <span className="text-gray-700">{order.amount.toFixed(5)}</span>
        <span className="text-right text-gray-700">{(order.total / 1000).toFixed(2)}</span>
      </div>
    );
  };
